...
def show_images_list(name, images_list, type='bgr', waitKey=0):
    out = image_hstack(images_list)
    if type == 'rgb':
        out = cv2.cvtColor(out, cv2.COLOR_RGB2BGR)  # 将BGR转为RGB
    cv2.imshow(name, out)
    cv2.waitKey(waitKey)
    return out
...
def pil_paste_image(im, mask, start_point=(0, 0)):
    ...
    out.paste(mask, start_point)  # fix a bug
